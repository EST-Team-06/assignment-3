...
Using the klee/klee:3.0 image, I ran the following command to generate test cases for `pow_client`:

```bash
clang -I /home/klee/klee_src/include/ -emit-llvm -c -g -O0 -Xclang -disable-O0-optnone pow.c -o pow.bc

klee -solver-backend=z3 -exit-on-error pow.bc 2>&1 | tee klee-output.txt
```

KLEE found the error in the first test case and stopped exploring further as the flag `-exit-on-error` was used. The concrete test case generated by KLEE was:
...
which corresponds to the call `pow_client(-1, 0)`.
The detailed outputs of KLEE can be found in [klee-results](klee-results/)